...
const posts = new Datastore({
    filename: path.resolve(process.cwd(), 'db/posts.db'),
    autoload: true
});
...
router.get('/:id', (req, res) => {
    posts.findOne({"id": parseInt(req.params.id)}, (err, doc) => {
        if(!err && doc !== null){
            res.status(200);
            res.json(doc);
            return
...
router.put('/:id', (req, res) => {
    posts.update({id: parseInt(req.params.id)}, req.body, () => {
        res.status(200);
        res.json({
...
router.delete('/:id', (req, res) => {
    posts.remove({id: parseInt(req.params.id)}, (err, doc) => {
        if (!err && doc !== null){
            res.status(200);
            res.json({
