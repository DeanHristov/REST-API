...
const users = new Datastore({
    filename: path.resolve(process.cwd(), 'db/users.db'),
    autoload: true
});
...
router.get('/:id', (req, res) => {
    users.findOne({"id": parseInt(req.params.id)}, (err, doc) => {
        if(!err && doc !== null){
            res.status(200);
            res.json(doc);
            return
...
router.put('/:id', (req, res) => {
    users.update({id: parseInt(req.params.id)}, req.body, () => {
        res.status(200);
        res.json({
...
router.delete('/:id', (req, res) => {
   users.remove({id: parseInt(req.params.id)}, (err, doc) => {
       if (!err && doc !== null){
           res.status(200);
           res.json({
